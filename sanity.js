...
async function requestSanity(query, params) {
  ...
  try {
    const requestUrl = getSanityUrl(query, params);
    console.log("Sanity request URL:", requestUrl);
    ...
    if (!response.ok) {
      ...
      console.error("Sanity request failed:", response.status, responseText);
...
async function fetchPortfolioProject(slug) {
  ...
  console.log("Sanity project result:", response.data);
  if (!response.data && !response.error) {
    console.log("Sanity project not found for slug:", slug);
    console.log("Sanity project query:", PROJECT_BY_SLUG_QUERY);
  }
